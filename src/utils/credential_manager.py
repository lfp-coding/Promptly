...
class CredentialManager:
    ...
    def _initialize_encryption(self):
        ...
        try:
            ...
            stored_key = keyring.get_password(
                "Promptly - Textprocessor", "encryption_key"
            )
            if stored_key:
            ...
            self._log_manager.log_error("Failed to initialize encryption", error=e)
            raise
    ...
    def store_api_key(self, api_provider: str, api_key: str):
        ...
        try:
            ...
            self._log_manager.log_error("Failed to save API key", error=e)
            raise
    ...
    def get_api_key(self, api_provider) -> str:
        ...
        try:
            api_clients = self._config_manager.get_value("api_clients")
            if (
                api_provider not in api_clients
                or not api_clients[api_provider].api_key_encrypted
            ):
                self._log_manager.log_warning(
                    f"No API key found for provider: {api_provider}"
                )
                return ""
            api_key = api_clients[api_provider].api_key_encrypted
            return self._cipher_suite.decrypt(api_key).decode()
        except Exception as e:
            self._log_manager.log_error("Failed to get API key", error=e)
            raise
    ...
    def delete_api_key(self):
        ...
        try:
            ...
            self._log_manager.log_error("Failed to delete API key", error=e)
    ...
    def validate_api_key(self, api_key: str) -> tuple[bool, str]:
        ...
        try:
            ...
            self._log_manager.log_error("Error validating API key", error=e)
            return False, error_msg
    ...
    def _list_credentials(self):
        ...
        try:
            ...
            self._log_manager.log_error("Failed to list credentials", error=e)
    ...
    def delete_credentials(self):
        ...
        try:
            ...
            self._log_manager.log_error(
                "Unexpected error deleting credentials", error=e
            )
