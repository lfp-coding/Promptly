...
class ConfigManager:
    ...
    def _load_config(self):
        ...
        try:
            ...
            self._log_manager.log_error("Failed to load configuration", error=e)
    ...
    def _save_config(self):
        ...
        try:
            ...
            self._log_manager.log_error("Failed to save configuration", error=e)
    ...
    def get_api_client(self):
        ...
        if api == "openai":
            if not self._config.api_clients[api].model:
                self._config.api_clients[api].model = "gpt-4o"
            return OpenAIClient().get_instance()

        elif api == "gemini":
            if not self._config.api_clients[api].model:
                self._config.api_clients[api].model = "gemini-2.0-flash"
    ...
    def _get_default_config(self):
        ...
        """
        general_config = GeneralConfig(autostart=False, api_provider="")

        api_clients = {}

        system_hotkeys = {
            "settings_window": SystemHotkey(
                hotkey="", hotkey_enabled=False, callback="open_settings"
            ),
            "chat_window": SystemHotkey(
                hotkey="", hotkey_enabled=False, callback="open_chat"
            ),
            "prompt_selector": SystemHotkey(
                hotkey="ctrl alt Y", hotkey_enabled=True, callback="open_context_menu"
            ),
        }
